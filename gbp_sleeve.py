...
HARD_STOP_PRICE = 1.3250      # Hard stop above the key political resistance
KILL_SWITCH_USD = -20000.0    # Capped kill switch for this specific 2% allocation
MAGIC_GBP = 20260707          # Unique magic number for tracking
STATE_FILE = "gbp_state.json"
...
def run_gbp_sleeve():
    """Main execution block for the GBP political trend-following short."""
    ...
    if state["cumulative_pnl"] <= KILL_SWITCH_USD:
        print("💀 KILL SWITCH TRIPPED. GBP sleeve halted forever.")
        return
    ...
    gbp_pos = [p for p in positions if p.magic == MAGIC_GBP]
